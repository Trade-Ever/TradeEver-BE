package com.trever.backend.user.service;
...
import com.trever.backend.user.repository.UserWalletRepository;
...
public class UserWalletService {
    ...
    @Transactional
    public void deposit(Long userId, Float amount) {
        if (amount <= 0) {
    ...
    @Transactional
    public void withdraw(Long userId, Float amount) {
        if (amount <= 0) {
    ...
    @Transactional
    public boolean hasSufficientFunds(Long userId, Float amount) {
        try {
            UserWallet wallet = getUserWallet(userId);
